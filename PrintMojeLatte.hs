...
instance Print AbsMojeLatte.Program where
  prt i = \case
    AbsMojeLatte.Program topdefs block -> prPrec i 0 (concatD [prt 0 topdefs, doc (showString "main"), prt 0 block])
...
instance Print AbsMojeLatte.Block where
  prt i = \case
    AbsMojeLatte.Block decls stmt -> prPrec i 0 (concatD [doc (showString "["), prt 0 decls, doc (showString "]"), doc (showString "{"), prt 0 stmt, doc (showString "}")])

instance Print AbsMojeLatte.Decl where
  prt i = \case
    AbsMojeLatte.Decl type_ item -> prPrec i 0 (concatD [prt 0 type_, prt 0 item])
...
instance Print AbsMojeLatte.Item where
  prt i = \case
    ...
    AbsMojeLatte.ArrInit id_ expr -> prPrec i 0 (concatD [prt 0 id_, doc (showString "["), prt 0 expr, doc (showString "]")])
...
instance Print AbsMojeLatte.Stmt where
  prt i = \case
    AbsMojeLatte.Seq stmt1 stmt2 -> prPrec i 0 (concatD [prt 0 stmt1, prt 0 stmt2])
